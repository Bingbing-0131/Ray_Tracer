...
//parseBezierCurve,praseRevSurface为完全拷贝，其余为独立完成
...
Material *SceneParser::parseMaterial() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    char filename[MAX_PARSER_TOKEN_LENGTH];  // 使用char数组而不是std::string
    filename[0] = 0;
    ...
    char texture_file[MAX_PARSER_TOKEN_LENGTH];  // 改为char数组
    texture_file[0] = 0;  // 初始化为空字符串
    ...
    while (true) {
        ...
        else if(strcmp(token, "diff_coef")==0){
            diff_coef = readFloat();
        } else if(strcmp(token, "spec_coef")==0){
            spec_coef = readFloat();
        ...
        // 添加纹理支持
            ...
            getToken(texture_file);  // 现在可以正确传递char数组
            has_texture = true;
            std::cout << "Found diffuse texture in material: " << texture_file << std::endl;
        }
        else if(strcmp(token, "bssrdf") == 0) {  // OBJ/MTL format support
            //getToken(texture_file);  // 现在可以正确传递char数组
            is_bssrdf = true;
            std::cout << "Found map_Kd texture in material: " << texture_file << std::endl;
        }
        else {
            assert (!strcmp(token, "}"));
            break;
    ...
    if (has_texture && strlen(texture_file) > 0) {
        texture_ptr = new Texture(texture_file);
        std::cout << "Created texture for material: " << texture_file << std::endl;
    } else {
        texture_ptr = new Texture();  // 创建默认纹理
    }

    // 创建材质对象，直接传递纹理指针
    auto *answer = new Material(diffuseColor, specularColor, shininess,
                               diff_coef, spec_coef, refractivity, refractIndex, 
                               emission, texture_ptr, is_glossy, rf, is_bssrdf);
...
RevSurface *SceneParser::parseRevSurface() {
    ...
    if (!strcmp(token, "BezierCurve")) {
        pCurve = parseBezierCurve();
    } else if (!strcmp(token, "BsplineCurve")) {
        pCurve = parseBsplineCurve();
    } else {
        printf("Unknown curve type in parseRevSurface: '%s'\n", token);
        exit(0);
    }
    getToken(token);
    assert(!strcmp(token, "}"));
    auto *answer = new RevSurface(pCurve, current_material);
    return answer;
}
